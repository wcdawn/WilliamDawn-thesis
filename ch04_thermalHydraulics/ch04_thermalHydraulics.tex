...
\section{Introduction}
  ...
  described in \sref{sec:geometry_description}, satisfy these assumptions with
...
\section{Power Normalization}
  ...
  eigenvector $\widetilde{\phi_{g,e}}$ (see \sref{sec:power_iterations}). The
  neutron flux calculated according to this method is an eigenvector and can be
  normalized to an arbitrary constant. For a specified total reactor power,
  $Q_{Rx}$, the normalization constant can be calculated. Given the
  un-normalized neutron flux as calculated by the power iteration method,
  $\widetilde{\phi_{g,e}}$, for  energy group $g$ in finite element $e$, the
...
\section{Axial Convection Model}
  ...
  \subsection{Geometric Model}
    ...
    channels is shown in \fref{fig:chunk_description}. Allow elevations to be
    indexed $k = 1,2,\ldots,N_z$ where $N_z$ is the total number of axial
    elevation. Then, each chunk has a unique $\{i,k\}$ combination and the total
    number of chunks is the product $N_{chunk} = N_{chan} \, N_z$.
  ...
  \subsection{Channel Mass Flow}
    ...
    per unit area is assumed to be uniform at the reactor inlet and the mass
    flow in a channel is the product of the reactor average mass flux and the
    channel flow area.  This partitioning of mass flow rate is chosen to allow
    for assemblies with different flow areas or the modeling of fractional
    assemblies in fractional core models.
  ...
  \subsection{Channel Enthalpy}
    ...
    within the channel given in \eref{eq:mass_flow_split}, and $q'_i(z)$ is the
    ...
    will be an input into the radial heat conduction model.
...
\section{Radial Conduction Model}
  ...
  average fuel rod in each fuel bundle. The model explicitly treats fuel, sodium
  bond, and cladding regions.  In the radial heat conduction model, it is
  ...
  \subsection{Surface Temperature and Centerline Temperatures}
    ...
    of cooling and a convective heat transfer correlation. For this work, the 
    ...
    \subsubsection{Temperature in Fuel}
      ...
      general, functional form form $k_F(T)$ \cite{fuelProp}.  For the assumed
      fuel composition of U-10Zr, the thermal conductivity is plotted in
      \fref{fig:kfuel_plot}.

      The steady-state heat conduction equation for the fuel temperature
      ...
      \end{equation}
      Now, \eref{eq:need_bc} can be evaluated.
      \begin{equation}
        \label{eq:dtdr_fuel}
      ...
      conductivity integral $K_F(T)$ and by employing a nonlinear search.  If,
      instead, $k_F(T)$ were constant such that $k_F(T) = \overline{k_F}$, then
    ...
    \subsubsection{Temperature in Sodium Bond}
      ...
      \sref{sec:temperature_in_fuel}, the expression \eref{eq:dtdr_fuel} is
      ...
      within the fuel in chunk $\{i,k\}$ specified in \eref{eq:chunkqppp_fuel}.
    ...
    \subsubsection{Clad Surface Temperature}
      ...
      axial heat convection model in \sref{sec:axial_convection_model},
      ...
      \end{align}
      With \eref{eq:tc}, all surface temperatures and the fuel centerline
  ...
  \subsection{Average Temperatures}
    ...
    from \sref{sec:surface_temps}. To calculate the average temperature in a
    region, an temperature function $T(r)$ is required. For the bond and clad
    regions, this is given in \eref{eq:tbr} and \eref{eq:tcr} respectively.
    ...
    using \eref{eq:tcl_constant_kfuel}. In a test case at reactor conditions,
...
\section{Cross Section Treatment}
  ...
  sections for use in the multigroup neutron diffusion equation. Cross sections
  are calculated by interpolating between temperatures in cross section
  libraries generated according to \sref{sec:cross_section_treatment} in order
  to simulate the temperature feedback effects as they relate to neutron cross 
  sections.
  ...
  Reactor material temperatures are updated periodically throughout the neutron
  ...
  of power iterations, see \sref{sec:power_iterations}.
  ...
  matrix (see \eref{eq:matrix_population}) which proves to be the most
  ...
  is commonly used in practice and no convergence problems have been observed.
  ...
  \begin{algorithm}
    ...
    \begin{algorithmic}[1]
      \State Complete $N_{iter}$ power iterations to calculate
        $\widetilde{\phi_{e,g}}$. Currently $N_{iter}=10$.
  ...
  \subsection{Coolant Cross Sections}
    ...
    described in \sref{sec:cross_section_treatment}, microscopic coolant cross 
  ...
  \subsection{Clad Cross Sections}
    ...
    fuel material and cladding material does not change temperature as extremely
    as other reactor materials. For this reason, a simple linear interpolation
    is used to interpolate macroscopic cross sections within this material. For
  ...
  \subsection{Bond Cross Sections}
    ...
    are calculated as \eref{eq:xs_cool}. This assumption is consistent with the
