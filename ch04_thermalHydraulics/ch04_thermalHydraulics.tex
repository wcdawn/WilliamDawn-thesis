...
\section{Radial Conduction Model}
  ...
  transported via gamma radiation will be absorbed directly in the coolant and
  ...
  \subsection{Surface Temperature and Centerline Temperatures}
    ...
    within bond and clad regions. Fuel centerline temperature must be solved
    implicitly due to the treatment of variable thermal conductivity. A
    numerical solution method is used to solve for the fuel centerline
    temperature.
    ...
    \subsubsection{Temperature in Fuel}
      ...
      fuel composition, the thermal conductivity is plotted in
      \fref{fig:kfuel_plot}.
      ...
      \end{gather}
      The fuel centerline temperature can be calculated using
      ...
      integral where $T_F(0)$ is the fuel centerline temperature, 
    ...
    \subsubsection{Temperature in Sodium Bond}
      ...
      \end{align}
      Recall from the earlier derivation of the fuel centerline temperature 
    ...
    \subsubsection{Clad Surface Temperature}
      ...
      is valid for a wide range of operating conditions common to metal cooled
      fast reactors \cite{subbotinUshakov}. The correlation relates the P\'eclet 
      number, $Pe$, to the Nusselt number, $Nu$. The P\'eclet number is the 
      product of the Prandlt number, $Pr$, and the Reynolds number, $Re$.
      \begin{equation}
        \label{eq:peclet}
        Pe = Re \, Pr
      \end{equation}
      ...
      \end{equation}
      where $S$ is the rod-pitch, $N_{rod}$ is the number of rods in the 
  ...
  \subsection{Average Temperatures}
    ...
    material regions. The derivations in this section require the fuel
    centerline and surface temperatures from \sref{sec:surface_temps}. To
    calculate the average temperature in a region, an temperature function
    $T(r)$ is required. For the bond and clad regions, this is given in
    \eref{eq:tbr} and \eref{eq:tcr} respectively.
    ...
    values, this approximation is acceptable.
    ...
    \subsubsection{Average Fuel Temperature}
      ...
      resolved in this simplified geometry due to the smearing of cross sections
      within a hexagonal fuel assembly. In addition, metal fuel has a lower 
...
\section{Cross Section Treatment}
  ...
  absorption resonances in fuel material, specifically uranium.
  ...
  for which the diffusion calculation will run before updating the temperatures.
  Currently, 10 power iterations are run per temperature update and this
  provides efficient and convergent behavior.  For a discussion of power
  iterations, see \sref{sec:power_iterations}.
  ...
  \subsection{Coolant Cross Sections}
    ...
    temperature $T_{n}<T_{\infty,i,k}<T_{n+1}$ and library temperatures
    $\{T_n\}$,
    \begin{equation}
      \label{eq:xs_cool}
