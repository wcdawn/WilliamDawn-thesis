...
\section{Introduction}
  This implementation of the Finite Element Method (FEM) has been compared
  ...
  the FEM. The strategy used comes from \cite{oberkampf}. The first step is 
...
\section{Error Analysis}
  ...
  within the problem domain, the FEM derived in \chref{ch:neutronDiffusion} is 
  second-order convergent in space \cite{textbookli} (Remark 7.13).
  ...
  proper implementation of the FEM must converge to the correct answer and do so
  at the correct rate.
...
\section{Two-Dimension Benchmark Solutions}
  ...
  \subsection{SNR}
    ...
    is a Sodium-cooled Fast Reactor (SFR) and operates principly with fast
    ...
    given by DIF3D are presented graphically in \fref{fig:diffusion_snr}
...
\section{Three-Dimension Benchmark Solutions}
  ...
  \begin{equation}
    ...
    \text{Rod Difference} \units{$\Delta k$} = (\keffsub{A} - \keffsub{x}) 
      \times 100 \%
  \end{equation}
  for $x = \{B,C\}$.
  ...
  \subsection{MONJU}
    ...
    three-dimension fast reactor benchmark problem is based on a SFR operating 
    principally with fast neutrons. Cross sections are provided for a 
