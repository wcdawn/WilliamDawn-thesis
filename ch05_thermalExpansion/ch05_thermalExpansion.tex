...
\section{Necessity of Modeling}
  ...
  \cite{ebriitests}. The \gls{ulohs}was demonstrated by shutting down coolant 
  pumps in the secondary coolant system, thereby disabling heat rejection from 
  the primary coolant system. No subsequent automatic or manual action was 
  taken. In the \gls{ulohs} demonstration, material temperatures do not peak and 
  instead quickly reduce below operating temperatures \cite{ebriitests}.

  The inherent safety demonstration test at \gls{ebr-ii} demonstrated the passive
...
\section{Model Details}
  ...
  \subsection{Material Properties}
    ...
    \eref{eq:lef_ht9} and \eref{eq:lef_u10zr} evaluate to zero near room
  ...
  \label{sec:model_details__assumptions_and_formulae}
    ...
    Given the assumptions of this model, a radial \gls{lef}can be defined using 
    \eref{eq:lef_ht9} as
    \begin{equation}
      \label{eq:lef_r}
    ...
    F_r(\texpstruct)}$.
    Similarly, an axial \gls{lef} can be defined using \eref{eq:lef_u10zr} as 
    \begin{equation}
      \label{eq:lef_a}
...
\section{Implementation}
  ...
  diffusion equation via the \gls{fem}
...
\section{Results}
  ...
  as $-1,000 \units{\glsentryshort{pcm}}$ at extreme temperatures. Additionally, in this model 
