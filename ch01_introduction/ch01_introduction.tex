...
\section{Cross Section Treatment}
  ...
  In this work, multigroup microscopic cross sections are generated using
  ...
  structure is used. \mcc solves the infinite-homogeneous (zero-dimension)
  neutron transport equation for isotopic number densities as input by the user.
  ...
  \chref{ch:thermalHydraulics}. Temperatures from the model are given in
  \tref{tab:xstemps}.
  ...
  Allow $N_{i,j}$ to be the number density in region ${i=1,2,\ldots,N_{reg}}$
  for isotope ${j=1,2,\ldots,N_{iso}}$. Then, the macroscopic cross section for
  ...
  \eref{eq:afrac_first} through \eref{eq:afrac_last}. Using these volume/area
  fractions, the macroscopic cross sections from \eref{eq:xs_homog} are
