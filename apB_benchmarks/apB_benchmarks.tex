...
\section{Two-Dimension Benchmark Problems}
  ...
  \subsection{IAEA}
    ...
    each of the four cases is summarized in \tref{tab:iaeakref}.  Assembly
...
\section{Three-Dimension Benchmark Problems}
  ...
  \subsection{MONJU}
    ...
    on the core periphery. Cross sections are provided in \tref{tab:monjuxs}. No
    fission spectrum is specified in the benchmark but the one used in these
    simulations is provided in \tref{tab:monjuchi}. In determining this fission
    ...
    extents of the problem. In pattern B, control rods in rings 5 and 6 (shaded
    in \fref{fig:monju_geom}) are half inserted and all others remain
    with drawn. In pattern C, the control assemblies in rings 5 and 6 that were
    previously half inserted are fully inserted. Control rod withdrawal patterns 
    are shown in \fref{fig:monju_cr_geom}.
  ...
  \subsection{KNK}
    ...
    provided in \tref{tab:knkxs_a} and \tref{tab:knkxs_b}. The fission spectrum
    is provided in \tref{tab:knkchi}.
