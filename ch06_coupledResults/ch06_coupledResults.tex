...
\label{sec:abr}
  ...
  \chref{ch:neutronDiffusion}. These methods agree to within 700 \units{pcm}. 
  The solution from the FEM as implemented in this work is $\keff = 1.006694$.
...
\label{sec:reactivity_coefficients}
  ...
  \label{sec:thermal_expansion_reactivity_coefficent}
    ...
    contribution to the power reactivity coefficient in FRs. This is due to two 
    ...
    fraction of neutrons created in the fuel due to fission that exit the core. 
    Light Water Reactors (LWRs) typically have low and ultra-low leakage designs
    with $\leakage \approx 2 \%$ \cite{textbookknief}. However, FRs simulated in 
    this work have $\leakage \approx 20\%$ and therefore are therefore highly 
    sensitive to decreases in fuel density and changing reactor dimensions due 
    ...
    expansion temperatres for the case with increased power, 
  ...
  \label{sec:coolant_temperature_reactivity_coefficient}
    ...
    coolant is not designed to moderate neutrons. Feedback in the coolant is due
    to two main phenomena: the decrease in absorption cross-sections in the
    coolant due to Doppler broadening and the decrease of density due to
    temperature increase. The dominant effect is the decrease of sodium density
    due to the temperature increase \cite{textbookknief}.

    Unlike all other reactivity coefficients presented here, the CTC is
    positive as is common in FRs. This implies increases in coolant temperature 
    will lead to an increase in reactivity and cause a subsequent increase in 
    reactor power. In FRs, the coolant acts as a parasitic neutron absorber. 
    Therefore, a decrease in the sodium absorption cross-section encourages 
    neutron absorption in fissile material in the fuel resulting in a reactivity 
    increase. This does not pose a stability problem as long as the power 
    reactivity coefficient remains negative.
