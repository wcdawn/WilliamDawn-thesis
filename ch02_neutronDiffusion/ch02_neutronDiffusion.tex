...
\section{Multigroup Neutron Diffusion Equation}
  ...
  \begin{conditions} % custom environment designed for this purpose
    ...
    $\Sigma_{t,g}(\vr)$ & macroscopic total cross section for energy group $g$ \units{$\frac{1}{\text{cm}}$}, \\
    $\widetilde{\chi_g}(\vr)$ & effective fission spectrum for energy group $g$,\\
    $\keff$ & effective neutron multiplication factor, \\
    $\nu \Sigma_{f,g}(\vr)$ & number of fission neutrons times macroscopic fission cross section in energy group $g$ \units{$\frac{1}{\text{cm}}$}, \\
    $\Sigma_{s,g' \rightarrow g} (\vr)$ & macroscopic scatter cross section from energy group $g'$ to energy group $g$ \units{$\frac{1}{\text{cm}}$}, \\
...
\section{Formulation of Finite Element Equations}
  ...
  \subsection{Derivation}
    ...
    \begin{figure}%[b]
      \centering
      \includegraphics[width=0.45\textwidth]{fixed0}
    ...
    \end{multline}
    Assuming the outward normal derivative is
    specified in the form of an albedo boundary condition with $\albedo$
    constant throughout the problem boundary as in \eref{eq:albedo_form2}.
    \begin{multline}
  ...
  \subsection{Matrix Quantities}
    ...
    \subsubsection{Linear Wedges}
      ...
      \begin{figure}
        \centering
        \includegraphics[width=0.45\textwidth]{Wref}
      ...
      surface. The matrix in \eref{eq:bigmat} is indexed $\mm_{ij}$ where
...
\section{Power Iterations}
  ...
  \subsection{Convergence of Power Iteration Method}
    ...
    \eref{eq:old_dominance_ratio} can be rewritten in terms of the eigenvalues
    ...
    Convergence criteria are then specified as an absolute 
    ...
    \eref{eq:gehin_r}. Instead, \eref{eq:gehin_notation} is solved using an
    iterative solution method. In this work, the \gls{fem} is used to solve
    \eref{eq:gehin_notation} as described in \sref{sec:formulation}. In the
    \gls{fem} representation, the quantity $\frac{1}{\keff} \mm \vPhi$ in
    \eref{eq:gehin_notation} is equivalent to the combined source
    representation, $q_{g,e}$, as described in \eref{eq:qelement_combined}.
    While the notation from the \gls{fem} could be rewritten for this proof, the
    notation from \eref{eq:gehin_notation} is more intuitive and compact.
    
  \subsection{Calculation of Source with Power Iterations}
  \label{sec:calculation_of_source_with_power_iterations}
    The source term in \eref{eq:gehin_notation}, $\frac{1}{\keff} \mm \vPhi$, is
    representative of the combined multigroup neutron source $q_g(\vr)$.  The
    multigroup neutron diffusion equation from \eref{eq:multigroup_source} can
...
\section{Implementation}
  ...
  visualized in the results VTK are shown in \fref{fig:flux_visualization} for
  two-dimensional and three-dimensional reactor simulations.

  \begin{figure}
    \centering
    \subfloat[Two-Dimensional, Sixth-Core Simulation.]{\includegraphics[width=0.40\textwidth]{snr_paraview}}
    \subfloat[Three-Dimensional, Full-Core Simulation.]{\includegraphics[width=0.40\textwidth]{monju3da_paraview}}
  ...
  \subsection{Memory and Storage}
    ...
    \begin{figure}
      \centering
      \includegraphics[width=0.4\textwidth]{adjacency_graph}
    ...
    \eref{eq:idx_number} (note that the value ``3'' is an arbitrary node 
    number).
    \begin{equation}
      \label{eq:idx_number}
      \left.
  ...
  \subsection{Linear System Solution}
    ...
    \end{equation}
    for $k \ge 1$.
    Eventually, the Krylov subspace from \eref{eq:cg_subspace} can be guaranteed
    ...
    \begin{algorithm}
      \caption{\glsentrylong{cg} Method \cite{Kelley1995IterativeEquations}.}
      \label{algorithm:CG}
      \begin{algorithmic}[1]
        \State $k = 0$
        \State $\vr = \vb - \ma \vx$
