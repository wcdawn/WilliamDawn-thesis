...
\section{Introduction}
  ...
  \eref{eq:continuous_diffusion} is given in \cite{duderstathamilton}. The
  ...
  It is crucial to note that the equations from \cite{duderstathamilton} are not
  ...
  coarse-group $E_g$.
  \begin{align}
    \sigma_{tr,g} &= \frac{\sum_{E_j \in E_g} \sigma_{tr,j} \varphi_j}
      {\sum_{E_j \in E_g} \varphi_j} \\
    \sigma_{t,g} &= \frac{\sum_{E_j \in E_g} \sigma_{t,j} \varphi_j}
      {\sum_{E_j \in E_g} \varphi_j} \\
    \nu \sigma_{f,g} &= \frac{\sum_{E_j \in E_g} \nu_{f,j} \sigma_{f,j}
      \varphi_j}{\sum_{E_j \in E_g} \varphi_j} \\
    \sigma_{s,g \rightarrow g'} &= \frac{\sum_{E_j' \in E_g'} \sum_{E_j \in E_g}
      \sigma_{s,j \rightarrow j'} \varphi_{j'}}{\sum_{E_j \in E_g} \varphi_j} \\
  ...
  Integrating \eref{eq:continuous_diffusion} over a single energy group, and
  ...
  will be discussed in \sref{sec:formulation}.
...
\section{Formulation of Finite Element Equations}
  ...
  \subsection{Matrix Quantities}
    ...
    \subsubsection{Linear Wedges}
      ...
      The
      relationship is given in \tref{tab:jacobi}.
  ...
  \subsection{Quadratures}
    ...
    \algref{algorithm:deriv_int}. The notation is dense as the method
    ...
    \algref{algorithm:deriv_int}, the quadrature has points $\{\vx_p\}$ and 
    weights $\{w_p\}$ for $p = 1,2,\ldots,N_P$ and the value of the integral is
    represented by \eref{eq:deriv_integral}.
    \begin{equation}
