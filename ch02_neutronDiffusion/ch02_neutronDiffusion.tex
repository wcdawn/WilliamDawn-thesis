...
\section{Multigroup Neutron Diffusion Equation}
  ...
  $q_{fiss,g}$ as in \eref{eq:qfiss}, an effective $\widetilde{\chi}$ for each
  ...
  \begin{equation}
    ...
    \widetilde{\chi_g}(\vr) = \frac{\sum_{i=1}^{N_{iso}} \chi_{i,g}(\vr)
      ...
      {\sum_{i=1}^{N_{iso}} \nu \Sigma_{f,i,g}(\vr) \phi_g(\vr)}
  \end{equation}
  Noting that \eref{eq:chi_collapse} requires the solution $\phi_g(\vr)$. In the
  \gls{fem} discretization, this means that $\chi_g$ must be calculated for each 
  finite element. Additionally, because \eref{eq:chi_collapse} requires a flux
...
\section{Formulation of Finite Element Equations}
  ...
  \subsection{Derivation}
    ...
    non-overlapping set of elements from \eref{eq:set_of_elements}.
    \begin{equation} 
