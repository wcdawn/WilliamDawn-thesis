...
\section{Formulation}
  \subsection{Derivation}
    ...
    \begin{equation} \label{eq:linear_combination}
      \phi_g(\vr) = \sum_{i=1}^{N} \beta_{g,i} \basis_i(\vr)
    \end{equation}
    Where coefficients $\{\beta_i\}$ are unknown and will be determined. 
    ...
    \begin{multline}
      \sum_{e=1}^{E} \albedo \sum_{i=1}^{N} \beta_{i,g}
        \int_{\partial \Omega_e}
        \basis_i(\vr)  \basis_j(\vr) \;ds +
        \sum_{e=1}^{E} D_{g,e} \sum_{i=1}^{N} \beta_{i,g}
        \int_{\Omega_e} \grad \basis_i(\vr) \cdot \grad \basis_i(\vr)\;d\Omega_e
        + \\
        \sum_{e=1}^{E} \Sigma_{r,g,e} \sum_{i=1}^{N} \beta_{i,g}
