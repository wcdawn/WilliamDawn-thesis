...
\section{Introduction}
  ...
  \begin{multline}\label{eq:continuous_diffusion}
    -\grad \cdot (D(\vr,E) \grad \varphi(\vr,E)) + \Sigma_t(\vr,E) \varphi(\vr,E) = \\
      \frac{\chi(\vr,E)}{\keff} \int_0^{\infty} \nu_f(\vr,E') \Sigma_f(\vr,E') 
      \varphi(\vr,E') \; dE' + \int_0^{\infty} \Sigma_s(\vr,E' \rightarrow E) 
      \varphi(\vr,E') \; dE'
  \end{multline}
  Where $D$ is the diffusion constant, $\varphi$ is the continuous scalar neutron flux, 
  $\Sigma_t$ is the total cross section, $\chi$ is the effective fission neutron 
  ...
  \begin{align}
    ...
    \Sigma_{s,g\rightarrow g'}(\vr) &= \frac{\int_{E_g'}^{E_{g'-1}} 
      \int_{E_g}^{E_{g-1}} \Sigma_s(\vr,E' \rightarrow E) \varphi(\vr,E')\;dE\;dE'}
  ...
  group $g$. \eref{eq:multigroup_source} is useful for solution of the
  multigroup neutron diffusion problem for an arbitrary number of groups. The
  equation is solved for each energy group and interaction between groups is
  described in the term $q_g(\vr)$. In other implementations, the problem can be
  solved for each group simultaneously using a method where interaction between
  groups is treated explicitly. By solving each group independently (as done
  here) the method remains general. Additionally, for many group energy
  structures as common to fast reactor applications solving each group
  independently is typically more computationally efficient.
...
\section{Formulation of Finite Element Equations}
  \subsection{Derivation}
    ...
    For selected elements, basis functions will be defined explicitly in
