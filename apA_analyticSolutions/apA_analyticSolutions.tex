...
\section{Introduction}
  ...
  method. A case matrix of the analytic solutions provided in this appendix is
  provided in \tref{tab:analytic_case_matrix}. A variety of cases are provided
  including fixed source and criticality problems. The number of spatial
  dimensions, number of energy groups, and number of materials are varied.

  \begin{table}
    \caption{Case Matrix for Analytic Solutions.}
    \label{tab:analytic_case_matrix}
    \begin{center}
      \begin{tabular}{lcccc}
        \toprule
  ...
  \begin{conditions}
    ...
    $\nu\Sigma_f$ & number of fission neutrons per unit neutron flux \units{$\frac{1}{\text{cm}}$}, \\
    $q_{fixed}$ & fixed volumetric neutron source \units{$\frac{1}{\text{cm}^3 s}$}.
  \end{conditions}
...
\section{One-Dimension, One-Group, Fixed Source}
  ...
  \eref{eq:1dfixed_c2} and can be substituted into \eref{eq:1dfixed_constants}.
  \begin{equation}
...
\section{One-Dimension, Two-Group, Criticality}
  ...
  \begin{align}
    \frac{k_2}{k_1} &= 0.2613242\footnotemark{}, \\
    \keff &= 0.892349.
  \end{align}
  \footnotetext{Incorrect in original thesis.}
