...
\section{Introduction}
  The following are analytic solutions designed for verification of
  ...
  \end{conditions}
   and $\grad^2 \phi = \grad \cdot (\grad \phi)$ and is used to
  simplify notation. \eref{eq:onegroup} is valid for problems with constant $D$
  as the constant can be removed from the derivative. For problems considered
  here, material constants such as $D$ are constant in some region, either the
  entire problem or a finite subdomain.
...
\section{Two-Dimension, One-Group, Criticality}
  ...
  \end{equation}
  $X(x)$ has solution similar to the form of the solution from
  \sref{sec:deriv_1d1g} as the boundary conditions are similar. Using the method 
  from \sref{sec:deriv_1d1g}, the fundamental mode is given as
  \begin{align}
...
\section{One-Dimension, One-Group, Two Region, Criticality}
  ...
  rewritten with a fuel buckling term $B_F$ as
  \begin{equation}
...
\section{Finite-Cylinder, One-Group, Criticality}
  ...
  $c_1$ is arbitrary so allow $c_1 = R_0$. Then the final solution for the
  function $R(r)$ is
  \begin{equation}
  ...
  the one-dimensional criticality problem presented in \sref{sec:1d1g}. This
  derivation is briefly repeated here. Recall \eref{eq:finite_cyl_zeq} which may
  be rewritten.
  \begin{equation}
