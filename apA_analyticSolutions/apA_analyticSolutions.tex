...
\section{One Dimension, One-Group, Fixed Source}
  ...
  \begin{align*}
    ...
    q_{fixed} &= 1
  \end{align*}
  Then, equation \eref{eq:onegroup} is written
  \begin{equation}
    - \grad^2 \phi + \phi = 1 
  \end{equation}
  or
...
\section{One Dimension, One Group, Criticality} \label{sc:onegroup1d}
  ...
  \eref{eq:onegroup} and the following coefficients:
  \begin{align*}
    D &= 1\\
    \Sigma_r &= 1\\
    \nu \Sigma_f &= 2\\
...
\section{One Dimension, Two Group, Criticality}
  ...
  \eref{eq:twogroupflux} into the second equation from \eref{eq:twogroup} 
  ...
  \end{equation}
  Returning to the first equation from \eref{eq:twogroup} and again dividing
...
\section{One-Dimension, One-Group, Two-Region, ``Reactor''}
  ...
  \eref{eq:onegroup}. Geometry is described in the figure below.
  \begin{figure}[H] 
    \centering
...
\section{One-Dimension, One-Group, Two-Region, Fixed-Source}
  ...
  The diffusion problem is written as \eref{eq:onegroup} with 
  $\nu \Sigma_f =0$ and $q_{fixed}$ nonzero. Then, from \eref{eq:particular},
  \begin{equation}
  ...
  \eref{eq:bc_xb} = \eref{eq:bc_flux} and solve for $c_4$.
  \begin{align}
    ...
    \phi_p \left(\frac{\cosh(B_1 a)}{\tanh(B_2(b-a))} - \frac{1}
      ...
      &= c_4
  \end{align}
  Setting the above expression equal to \eref{eq:bc_current} and finally 
  solve for $c_2$.
  \begin{align}
  ...
  \end{align}
  Now, use equations \eref{eq:bc_x0}, \eref{eq:c2}, \eref{eq:bc_flux}, and 
  \eref{eq:bc_current} to solve for $c_1$ through $c_4$ respectively.
