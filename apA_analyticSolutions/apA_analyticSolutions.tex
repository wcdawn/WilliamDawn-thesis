...
\section{Introduction}
  ...
  notation. This is useful for problems with constant $D$ as the constant can be
...
\section{One-Dimension, One-Group, Fixed Source}
  ...
  \begin{equation}
    ...
    \frac{d^2}{dx^2} \phi_x(x) - \kappa^2 \phi_H(x) = 0.
  \end{equation}
  ...
  \end{equation}
  A typical result of \eref{eq:analytic_1dfixedsrc} is presented in
...
\section{One-Dimension, One-Group, Criticality} 
  ...
  $c_2 = \phi_0$. Then, recall the zeros of the sine function.
  \begin{align}
    B_g \, L_x &= n \, \pi \\
    B_g &= \frac{n \, \pi}{L_x}
  \end{align}
  ...
  the flux is normalized to have the maximum equal to the fixed solution for 
...
\section{Two-Dimension, One-Group, Criticality}
  ...
  geometry is then expanded into partial derivatives.
  \begin{equation}
  ...
  \begin{align}
    ...
    \therefore \; c_1 &= 0
  \end{align}
  Inserting this result into \eref{eq:2d1g_x_general} gives
  \begin{equation}
    \label{eq:2d1g_x_sin}
    X(x) = c_2 \, \sin( \alpha x).
  \end{equation}
  ...
  allow $c_2 = \phi_0$. Then, recall the zeros of the sine function.
  \begin{align}
    \alpha \, L_x &= n \, \pi \\
    \alpha &= \frac{n \, \pi}{L_x}
  \end{align}
...
\section{One-Dimension, Two-Group, Criticality}
  ...
  Since this is a bare core problem ($\phi=0$ on all boundaries), the general
  solution to \eref{eq:1d2g_1} and \eref{eq:1d2g_2} has the form common to
  one-group bare core problems.
  \begin{equation}
  ...
  \begin{align}
    ...
    &= c_g \, \sin(B \, L_x)
  \end{align}
  The buckling $B$ is then specified by the zeros of the $\sin()$ function. 
  \begin{equation}
    \label{eq:1d2g_buckle_geom}
    B = \frac{\pi}{L_x}
  \end{equation}
  To find the unknown coefficients $c_g$, differentiate \eref{eq:1d2g_sin}
  twice.
  \begin{equation}
    \label{eq:1d2g_sin_d2}
    \frac{d^2}{dx^2} \phi_g(x) = - B^2 \, c_g \, \sin(B\,x)
  \end{equation}
  Insert \eref{eq:1d2g_sin} and \eref{eq:1d2g_sin_d2} into \eref{eq:1d2g_1} and
  \eref{eq:1d2g_2}.
  \begin{align}
    D_1 \, B^2 \, c_1 \, \sin(B\,x) + \Sigma_{r1} \, c_1 \, \sin(B\,x) &=
      \frac{1}{\keff} \left( \nu \Sigma_{f1} \, c_1 \, \sin(B \, x) + \nu
      \Sigma_{f2} \, c_2 \, \sin(B\,x) \right) \\
    D_2 \, B^2 \, c_2 \sin(B\,x) + \Sigma_{r2} \, c_2 \, \sin(B\,x) &=
      \Sigma_{s 1\rightarrow 2} \, c_1 \, \sin(B\,x)
  \end{align}
  Dividing through the equations by $\sin(B\,x)$.
  \begin{align}
    \label{eq:1d2g_expression1}
    D_1 \, B^2 \,c_1+ \Sigma_{r1}\,c_1 &= \frac{1}{\keff} \left( \nu \Sigma_{f1}
      \, c_1 + \nu \Sigma_{f2} \, c_2 \right) \\
    \label{eq:1d2g_expression2}
    D_2 \, B^2 \, c_2 + \Sigma_{r2}\,c_2 &= \Sigma_{s 1\rightarrow 2} \, c_1
  \end{align}
  ...
  \begin{align}
    \keff &= \frac{\nu \Sigma_{f1} \, c_1 + \nu \Sigma_{f2} \, c_2}
      {D_1 \, B^2 c_1 + \Sigma_{r1} \, c_1 }\\
    \keff &= \frac{\nu \Sigma_{f1} \, c_1 + \nu \Sigma_{f2} \, c_2}
      {D_1 \, \left(\frac{\pi}{L_x}\right)^2 \, c_1 + \Sigma_{r1} \, c_1}
  \end{align}
  Divide the top and bottom of the ratio by $c_1$.
  \begin{equation}
    \label{eq:1d2g_keff}
  ...
  \begin{align}
    ...
    \phi_2(x) &= \frac{c_2}{c_1} \, \phi_0 \, \sin\left(\frac{\pi}{L_x} x
      \right)
  \end{align}
  where $\phi_0$ is an arbitrary constant.
  An example solution is plotted in \fref{fig:1d2g}. The coefficient $c_1$ is
  ...
  VVER440 benchmark in \sref{sec:vver440} MAT1 are used for this problem. For 
  ease of reference, the cross sections are also presented in 
...
\section{One-Dimension, One-Group, Two Region, Criticality}
  ...
  material at the edge of the reactor. The purpose of this problem is to test
  the materials mapping of the solver. The problem is in the domain 
  ...
  \end{align}
  Leading to the transcendental function
  \begin{equation}
    \label{eq:2reg_bf}
    B_F \, \tan(B_F \, L_F) = \frac{D_R \, B_R}{D_F \, \tanh(B_R \, (L_R-L_F))}
  \end{equation}
  where $B_R$ is defined according to \eref{eq:2regR_b2}. 
  ...
  such as MATLAB's \verb|vpasolve()| or a generic bisection search must be used.
  The $\tan()$ function causes the solution $B_F$ to be especially sensitive to 
...
\section{Finite-Cylinder, One-Group, Criticality}
  ...
  solution. This problem is located in $r \in [0,T]$ and $z \in [0,H]$. The 
  ...
  trivial and $\gamma$ specified by geometry. 
  \begin{align}
    \gamma H &= n \pi \\
    \gamma &= \frac{n \pi}{H} \\
  ...
  arbitrary so allow $c_4 = \phi_0$. Then
  \begin{equation}
  ...
  \begin{equation}
    ...
    \phi(r,z) = J_0\left(\frac{\alpha_0}{T} r\right) \sin\left(\frac{\pi}{H} z
      \right)
  \end{equation}
  An example flux plot is shown in \fref{fig:finite_cyl}.
